...
{
    NVL_SUPPRESS_UNUSEDVARIABLE(argc);
    NVL_SUPPRESS_UNUSEDVARIABLE(argv);
    ...
    nvl::BST<nvl::Point2d, nvl::Point2d, CustomPointComparator> bst3(
                CustomPointComparator(nvl::Point2d(1,0)));
	//Adding points
    bst3.insert(nvl::Point2d(1,2));
    bst3.insert(nvl::Point2d(2,5));
    bst3.insert(nvl::Point2d(-4,5));
    bst3.insert(nvl::Point2d(8,8));

	//Normal iteration
	std::cout << "The Tree contains:" << std::endl << "    ";
    for (const nvl::Point2d& point : bst3) {
    ...
    nvl::BST<nvl::Point2d> bst4(
                [](const nvl::Point2d& x, const nvl::Point2d& y)
	{
		return x > y;
	});
	//Adding points
    bst4.insert(nvl::Point2d(1,2));
    bst4.insert(nvl::Point2d(2,5));
    bst4.insert(nvl::Point2d(-4,5));
    bst4.insert(nvl::Point2d(8,8));

	//Normal iteration

	std::cout << "The Tree contains:" << std::endl << "    ";
    for (const nvl::Point2d& point : bst4) {
        std::cout << "[" << point.x() << "," << point.y() << "] ";
    }
	std::cout << std::endl;
