...
int main(int argc, char *argv[])
{
    ...
    for (Mesh::VertexId j = 0; j < face.vertexNumber(); ++j) {
        ...
        polygon[j] = v.point();
        redValues[j] = mesh.vertexColor(v).redF();
        greenValues[j] = mesh.vertexColor(v).greenF();
        blueValues[j] = mesh.vertexColor(v).blueF();
    }
    ...
    for (Mesh::Vertex& v : subdividedMesh1.vertices()) {
        ...
        //Set the color
        subdividedMesh1.setVertexColor(v, nvl::Color(red, green, blue));
    }
    ...
    for (Mesh::Vertex& v : subdividedMesh2.vertices()) {
        ...
        //Set the color
        subdividedMesh2.setVertexColor(v, nvl::Color(red, green, blue));
    }
